...
exports.updateMongoParams = function(params) {
  if (params.so == 'desc') {
    params.so = -1;
  }
  ...
  if (params.ps) {
    params.ps = Number(params.ps);
  }

  return params;
};

exports.createMongoFind = function(params) {
  var find = {};
  var searchParams = [];
  _.each(_.keys(params.search), function(key) {
    var hash = {};

    if(_.isString(params.search[key])) {
      if(params.search[key].search(/[^0-9]+/) === -1) {
        // convert any numbers to a number data type
        hash[key] = Number(params.search[key]);
      } else if(params.search[key] !== '%') {
        // assume we aren't looking for exact match
        hash[key] = new RegExp('.*' + params.search[key] + '.*');
      } else {
        hash[key] = params.search[key].toLowerCase();
      }
    } else {
      hash[key] = params.search[key];
    }
    searchParams.push(hash);
  });
  if(searchParams.length) {
    find['$and'] = searchParams;
  }
  return find;
};
